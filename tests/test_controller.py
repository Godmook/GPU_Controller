...
class TestResourceView(unittest.TestCase):
    ...
    def test_get_workload_dominant_share(self):
        ...
        dominant_share = self.resource_view.get_workload_dominant_share(
            workload_resources
        )
    ...
    def test_can_schedule_workload(self):
        """스케줄링 가능성 테스트"""
        # 사용 가능한 리소스 설정
        self.resource_view._cluster_capacity = {"cpu": 100.0, "nvidia.com/gpu": 10.0}
        self.resource_view._cluster_usage = {"cpu": 50.0, "nvidia.com/gpu": 5.0}

        # 스케줄링 가능한 Workload
        schedulable_workload = {"cpu": 30.0, "nvidia.com/gpu": 3.0}
        self.assertTrue(self.resource_view.can_schedule_workload(schedulable_workload))

        # 스케줄링 불가능한 Workload
        unschedulable_workload = {"cpu": 100.0, "nvidia.com/gpu": 3.0}  # 용량 초과
        self.assertFalse(
            self.resource_view.can_schedule_workload(unschedulable_workload)
        )
