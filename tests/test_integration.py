...
class TestControllerIntegration:
    ...
    def test_workload_priority_update_cycle(self):
        ...
        with patch('controller.controller.KubernetesClient') as mock_client:
            ...
            self.controller.initialize()
            result = self.controller._update_workload_priorities()
            
            assert result is True
            mock_client.return_value.update_workload_priority.assert_called_once()
...
class TestEndToEndWorkflow:
    ...
    def test_priority_calculation_workflow(self, sample_workloads, priority_calculator):
        ...
        # 긴급 워크로드 우선순위 계산
        urgent_priority = priority_calculator.calculate_priority(urgent_workload)
        assert urgent_priority.tier == PriorityTier.URGENT
        assert urgent_priority.final_priority > 0
        
        # 일반 워크로드 우선순위 계산
        normal_priority = priority_calculator.calculate_priority(normal_workload)
        assert normal_priority.tier == PriorityTier.NORMAL
    ...
    def test_resource_allocation_workflow(self, sample_workloads, mock_resource_view):
        """리소스 할당 워크플로우 테스트"""
        workload = sample_workloads['normal_workload']
        
        # 리소스 추출
        resources = mock_resource_view.extract_workload_resources(workload)
        assert resources['cpu'] == 4.0  # 2 * 2 pods
        assert resources['nvidia.com/gpu'] == 2.0  # 1 * 2 pods
        
        # 스케줄링 가능성 확인
        can_schedule = mock_resource_view.can_schedule_workload(resources)
        assert can_schedule is True
        
        # Dominant share 계산
        dominant_share = mock_resource_view.get_workload_dominant_share(resources)
...
class TestPerformanceIntegration:
    ...
    def test_large_workload_processing(self, priority_calculator):
        ...
        for workload in workloads:
            priority = priority_calculator.calculate_priority(workload)
        ...
        for priority in priorities:
            ...
            assert priority.tier in [PriorityTier.NORMAL, PriorityTier.APPROVED, PriorityTier.URGENT]
        ...
        with patch('controller.k8s_client.config') as mock_config:
            with patch('controller.k8s_client.client') as mock_client:
                ...
                mock_client.CoreV1Api.return_value.list_node.return_value.items = [
                    Mock(spec=Mock(taints=[]), status=Mock(allocatable={
                        'cpu': '100',
                        'memory': '1000Gi',
                        'nvidia.com/gpu': '10'
                    }))
...
class TestErrorHandlingIntegration:
    ...
    def test_kubernetes_connection_failure(self):
        ...
        with patch('controller.k8s_client.config') as mock_config:
            mock_config.load_incluster_config.side_effect = Exception("Connection failed")
    ...
    def test_invalid_workload_data(self, priority_calculator):
        ...
        for workload in invalid_workloads:
            try:
                ...
                assert priority.tier == PriorityTier.NORMAL
                assert priority.final_priority >= 0
    ...
    def test_resource_overflow_handling(self, mock_resource_view):
        ...
        try:
            dominant_share = mock_resource_view.get_workload_dominant_share(large_resources)
            # 무한대 값이 적절히 처리되어야 함
            assert dominant_share > 0
        except Exception as e:
            # 예외가 발생해도 적절히 처리되어야 함
