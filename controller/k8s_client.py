...
class KubernetesClient:
    ...
    def create_priority_class(self, priority_class_name: str, value: int, description: str) -> bool:
        ...
        try:
            ...
            logger.info(f"Created Priority Class: {priority_class_name} with value {value}")
            return True
            
        except ApiException as e:
            if e.status == 409:  # Already exists
                logger.debug(f"Priority Class {priority_class_name} already exists")
                return True
            else:
                logger.error(f"Failed to create Priority Class {priority_class_name}: {e}")
                return False
    ...
    def ensure_priority_classes(self) -> bool:
        ...
        try:
            for class_name, class_config in Config.KUEUE_PRIORITY_CLASSES.items():
                success = self.create_priority_class(
                    class_name,
                    class_config["value"],
                    class_config["description"]
                )
                if not success:
                    return False
    ...
    def update_workload_priority_class(self, workload_name: str, namespace: str, priority_class_name: str) -> bool:
        ...
        try:
            ...
            current_workload["metadata"]["annotations"]["wdrf.x-k8s.io/priority-class"] = priority_class_name
            current_workload["metadata"]["annotations"]["wdrf.x-k8s.io/priority-updated"] = "true"
            ...
            logger.info(f"Updated Priority Class for workload {workload_name} to {priority_class_name}")
            return True
    ...
    def _is_gang_scheduling_workload(self, workload: Dict[str, Any]) -> bool:
        ...
        try:
            ...
            for pod_set in pod_sets:
                ...
                # Pod Group 정보 확인
                if (labels.get("kueue.x-k8s.io/pod-group-name") and 
    ...
    def _workload_belongs_to_pod_group(self, workload: Dict[str, Any], pod_group_name: str) -> bool:
    ...
    def update_workload_priority(self, workload_name: str, namespace: str, priority: int) -> bool:
        ...
        try:
            ...
            current_workload["metadata"]["annotations"]["wdrf.x-k8s.io/priority"] = str(priority)
            current_workload["metadata"]["annotations"]["wdrf.x-k8s.io/priority-updated"] = "true"
    ...
    def get_local_queues(self, namespace: str = None) -> List[Dict[str, Any]]:
        """LocalQueue를 조회합니다."""
        try:
            if namespace:
