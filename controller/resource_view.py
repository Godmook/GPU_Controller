...
class ResourceView:
            ...
            for resource_name, quantity in allocatable.items():
                if resource_name in Config.RESOURCE_WEIGHTS:
                    try:
                        ...
                        logger.warning(f"Failed to parse quantity for {resource_name}: {quantity}")
        ...
        for resource_name, required in workload_resources.items():
            if resource_name in Config.RESOURCE_WEIGHTS:
                if available.get(resource_name, 0.0) < required:
                    logger.debug(f"Cannot schedule workload: insufficient {resource_name}")
                    return False
        
        return True
    
    def get_workload_dominant_share(self, workload_resources: Dict[str, float]) -> float:
    ...
    def calculate_resource_efficiency(self, workload_resources: Dict[str, float]) -> float:
