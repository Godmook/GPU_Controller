...
class PriorityCalculator:
    ...
    def calculate_workload_priority(self, workload: Dict[str, Any]) -> WorkloadPriority:
        ...
        try:
            ...
            logger.error(f"Failed to calculate priority for workload {workload.get('metadata', {}).get('name', 'unknown')}: {e}")
            # 기본값 반환
            return WorkloadPriority(
    ...
    def _get_creation_time(self, workload: Dict[str, Any]) -> float:
        ...
        try:
            ...
            if "creationTimestamp" in workload["metadata"]:
                ...
                creation_dt = datetime.datetime.fromisoformat(creation_str.replace('Z', '+00:00'))
    ...
    def _determine_priority_tier(self, workload: Dict[str, Any]) -> PriorityTier:
        ...
        # 높은 우선순위 조건들
        if (annotations.get("wdrf.x-k8s.io/urgent", "false").lower() == "true" or
            annotations.get("wdrf.x-k8s.io/approved", "false").lower() == "true" or
            annotations.get("wdrf.x-k8s.io/high-priority", "false").lower() == "true"):
    ...
    def _extract_gang_scheduling_info(self, workload: Dict[str, Any]) -> Dict[str, Any]:
        ...
        try:
            ...
            for pod_set in pod_sets:
                ...
                pod_group_total_count = annotations.get("kueue.x-k8s.io/pod-group-total-count")
    ...
    def _calculate_aging_factor(self, waiting_time: float) -> float:
        ...
        logger.debug(f"Aging factor calculated: {aging_factor} (waiting time: {waiting_time}s)")
        return aging_factor
    
    def _calculate_final_priority(self, priority_tier: PriorityTier, dominant_share: float, aging_factor: float) -> float:
        # 중요도(Tier)별로 Priority Class를 이미 부여
        # 동일 Tier 내에서는 dominant_share - aging_factor가 작을수록 우선
        return dominant_share - aging_factor
    
    def sort_workloads_by_priority(self, workloads: List[Dict[str, Any]]) -> List[WorkloadPriority]:
        workload_priorities = []
        ...
        workload_priorities.sort(key=lambda x: (x.priority_tier.value, x.final_priority))
        logger.info(f"Sorted {len(workload_priorities)} workloads by priority")
        return workload_priorities
    
    def get_priority_summary(self, workload_priorities: List[WorkloadPriority]) -> Dict[str, Any]:
    ...
    def should_override_priority(self, workload: Dict[str, Any]) -> bool:
        ...
        annotations = workload.get("metadata", {}).get("annotations", {})
        return annotations.get("wdrf.x-k8s.io/priority-override", "false").lower() == "true"
