...
class WDRFController:
        ...
        try:
            while self.running:
                ...
                try:
                    ...
                    logger.info(f"Cycle {self.stats['total_cycles']} completed in "
    ...
    def _process_regular_workloads(self, workloads: List[Dict[str, Any]]):
        ...
        # 우선순위 계산 및 정렬
        workload_priorities = self.priority_calculator.sort_workloads_by_priority(workloads)
        ...
        for i, workload_priority in enumerate(workload_priorities):
            try:
                ...
                if success:
                    ...
                    logger.debug(f"Updated Priority Class for {workload_priority.workload_name} "
                ...
                logger.error(f"Error updating priority for {workload_priority.workload_name}: {e}")
        
        self.stats["total_priority_updates"] += updates_count
        self.stats["total_priority_class_updates"] += priority_class_updates
        logger.info(f"Updated priorities for {updates_count}/{len(workload_priorities)} workloads")
        logger.info(f"Updated Priority Classes for {priority_class_updates}/{len(workload_priorities)} workloads")
    
    def _log_cycle_summary(self, all_workloads: List[Dict[str, Any]], gang_workloads: List[Dict[str, Any]]):
        """사이클 요약을 로그로 출력합니다."""
        if not all_workloads:
            return
        
        # 우선순위 요약
        workload_priorities = self.priority_calculator.sort_workloads_by_priority(all_workloads)
        priority_summary = self.priority_calculator.get_priority_summary(workload_priorities)
        ...
        logger.info(f"Priority distribution: {priority_summary['priority_distribution']}")
        logger.info(f"Average waiting time: {priority_summary['average_waiting_time']:.1f}s")
        logger.info(f"Average dominant share: {priority_summary['average_dominant_share']:.3f}")
        ...
        if workload_priorities:
            ...
            for i, wp in enumerate(workload_priorities[:3]):
                logger.info(f"  {i+1}. {wp.workload_name} "
    ...
    def get_controller_stats(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "total_gang_scheduling_processed": self.stats["total_gang_scheduling_processed"],
            "last_cycle_time": self.stats["last_cycle_time"],
            "average_cycle_time": (uptime / self.stats["total_cycles"]) if self.stats["total_cycles"] > 0 else 0,
    ...
    def shutdown(self):
        ...
        logger.info(f"Total priority class updates: {stats['total_priority_class_updates']}")
        logger.info(f"Total gang scheduling processed: {stats['total_gang_scheduling_processed']}")
